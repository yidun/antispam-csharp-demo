using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;
using System.Net.Http;

namespace Com.Netease.Is.Antispam.Demo
{
    class ImageCheckV5ApiDemo
    {
        public static void imageCheck()
        {
            /** 产品密钥ID，产品标识 */
            String secretId = "your_secret_id";
            /** 产品私有密钥，服务端生成签名信息使用，请严格保管，避免泄露 */
            String secretKey = "your_secret_key";
            /** 业务ID，易盾根据产品业务特点分配 */
            String businessId = "your_business_id";
            /** 易盾反垃圾云服务图片在线检测接口地址 */
            String apiUrl = "http://as.dun.163.com/v5/image/check";
            Dictionary<String, String> parameters = new Dictionary<String, String>();

            long curr = (long)(DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalMilliseconds;
            String time = curr.ToString();

            // 1.设置公共参数
            parameters.Add("secretId", secretId);
            parameters.Add("businessId", businessId);
            parameters.Add("version", "v5");
            parameters.Add("timestamp", time);
            parameters.Add("nonce", new Random().Next().ToString());

            // 2.设置私有参数
            JArray jarray = new JArray();
            JObject image1 = new JObject();
            image1.Add("name", "https://nos.netease.com/yidun/2-0-0-a6133509763d4d6eac881a58f1791976.jpg");
            image1.Add("type", 1);
            image1.Add("data", "https://nos.netease.com/yidun/2-0-0-a6133509763d4d6eac881a58f1791976.jpg");
            jarray.Add(image1);

            JObject image2 = new JObject();
            image2.Add("name", "{\"imageId\": 33451123, \"contentId\": 78978}");
            image2.Add("type", 2);
            image2.Add("data", "");
            jarray.Add(image2);

            parameters.Add("images", jarray.ToString());
            // parameters.Add("account", "csharp@163.com");
            // parameters.Add("ip", "123.115.77.137");

            // 3.生成签名信息,指定国密SM3加密
            // parameters.Add("signatureMethod", "SM3");
            String signature = Utils.genSignature(secretKey, parameters.GetValueOrDefault("signatureMethod", "MD5"), parameters);
            parameters.Add("signature", signature);

            // 4.发送HTTP请求
            HttpClient client = Utils.makeHttpClient();
            String result = Utils.doPost(client, apiUrl, parameters, 10000);
            if(result != null)
            {
                JObject ret = JObject.Parse(result);
                int code = ret.GetValue("code").ToObject<Int32>();
                String msg = ret.GetValue("msg").ToObject<String>();
                if (code == 200)
                {
                    JArray resultArray = (JArray)ret.SelectToken("result");
                    foreach (var item in resultArray)
                    {
                        JObject tmp = (JObject)item;
                        JObject antispam = tmp.GetValue("antispam").ToObject<JObject>();
                        String name = antispam.GetValue("name").ToObject<String>();
                        int status = antispam.GetValue("status").ToObject<Int32>();
                        String taskId = antispam.GetValue("taskId").ToObject<String>();
                        if(status == 2){
                            int suggestion = antispam.GetValue("suggestion").ToObject<Int32>();
                            JArray labels = (JArray)antispam.SelectToken("labels");
                            Console.WriteLine(String.Format("taskId={0}，status={1}，name={2}，suggestion={3}", taskId, status, name, suggestion));
                            // 产品需根据自身需求，自行解析处理，本示例只是简单判断分类级别
                            foreach (var lable in labels)
                            {
                                JObject lableData = (JObject)lable;
                                int label = lableData.GetValue("label").ToObject<Int32>();
                                int level = lableData.GetValue("level").ToObject<Int32>();
                                double rate = lableData.GetValue("rate").ToObject<Double>();
                                // 返回二级分类信息，根据需要解析
                                JArray subLabels = (JArray)lableData.SelectToken("subLabels");
                                Console.WriteLine(String.Format("label:{0}, level={1}, rate={2}, subLabels={3}", label, level, rate, subLabels));
                            }
                            switch (suggestion) {
                                case 0:
                                    Console.WriteLine("#图片机器检测结果：最高等级为\"正常\"\n");
                                    break;
                                case 1:
                                    Console.WriteLine("#图片机器检测结果：最高等级为\"嫌疑\"\n");
                                    break;
                                case 2:
                                    Console.WriteLine("#图片机器检测结果：最高等级为\"确定\"\n");
                                    break;
                                default:
                                    break;
                            }
                        }else {
                            Console.WriteLine(String.Format("图片检测失败，taskId={0}，status={1}，name={2}", taskId, status, name));
                        }
                        // ocr结果
                        if(null!=tmp.GetValue("ocr")){
                            JObject ocr = tmp.GetValue("ocr").ToObject<JObject>();
                            JArray ocrDetails = (JArray)ocr.SelectToken("details");
                            foreach (var ocrDetail in ocrDetails)
                            {
                                JObject lObject = (JObject)ocrDetail;
                                String content = lObject.GetValue("content").ToObject<String>();
                                JArray lineContents = (JArray)lObject.SelectToken("lineContents");
                                Console.WriteLine(String.Format("识别ocr文本内容:{0}, ocr片段及坐标信息:{1}", content, lineContents));
                            }
                        }
                        // 人脸信息
                        if(null!=tmp.GetValue("face")){
                            JObject face = tmp.GetValue("face").ToObject<JObject>();
                            JArray faceDetails = (JArray)face.SelectToken("details");
                            foreach (var faceDetail in faceDetails)
                            {
                                JObject lObject = (JObject)faceDetail;
                                String faceNumber = lObject.GetValue("faceNumber").ToObject<String>();
                                JArray faceContents = (JArray)lObject.SelectToken("faceContents");
                                Console.WriteLine(String.Format("识别人脸数量:{0}, 人物信息及坐标信息:{1}", faceNumber, faceContents));
                            }
                        }
                        // 图片质量检测结果
                        if(null!=tmp.GetValue("quality")){
                            JObject quality = tmp.GetValue("quality").ToObject<JObject>();
                            JArray qualityDetails = (JArray)quality.SelectToken("details");
                            foreach (var qualityDetail in qualityDetails)
                            {
                                JObject lObject = (JObject)qualityDetail;
                                float aestheticsRate = lObject.GetValue("aestheticsRate").ToObject<float>();
                                float sharpnessRate = lObject.GetValue("sharpnessRate").ToObject<float>();
                                JObject metaInfo = lObject.GetValue("metaInfo").ToObject<JObject>();
                                JObject boarderInfo = lObject.GetValue("boarderInfo").ToObject<JObject>();
                                JObject backgroundInfo = lObject.GetValue("backgroundInfo").ToObject<JObject>();
                                Console.WriteLine(String.Format("图片美观度分数:{0}, 图片清晰度分数:{1}, 图片基本信息:{2}, 图片边框信息:{3}, 图片背景信息:{4}", aestheticsRate, sharpnessRate, metaInfo, boarderInfo, backgroundInfo));
                            }
                        }
                        // 图片logo结果
                        if(null!=tmp.GetValue("logo")){
                            JObject logo = tmp.GetValue("logo").ToObject<JObject>();
                            JArray logoDetails = (JArray)logo.SelectToken("details");
                            foreach (var logoDetail in logoDetails)
                            {
                                JObject lObject = (JObject)logoDetail;
                                String logoName = lObject.GetValue("logoName").ToObject<String>();
                                float x1 = lObject.GetValue("x1").ToObject<float>();
                                float y1 = lObject.GetValue("y1").ToObject<float>();
                                float x2 = lObject.GetValue("x2").ToObject<float>();
                                float y2 = lObject.GetValue("y2").ToObject<float>();
                                Console.WriteLine(String.Format("logo信息:{0}, 左上角点x坐标:{1}, 左上角点y坐标:{2}, 右下角点x坐标:{3}, 右下角点y坐标:{4}", logoName, x1, y1, x2, y2));
                            }
                        }
                        // 图片识别结果
                        if(null!=tmp.GetValue("discern")){
                            JObject discern = tmp.GetValue("discern").ToObject<JObject>();
                            JArray discernDetails = (JArray)discern.SelectToken("details");
                            foreach (var discernDetail in discernDetails)
                            {
                                JObject lObject = (JObject)discernDetail;
                                int type = lObject.GetValue("type").ToObject<int>();
                                String discernName = lObject.GetValue("discernName").ToObject<String>();
                                float rate = lObject.GetValue("rate").ToObject<float>();
                                switch (type) {
                                    case 1:
                                        Console.WriteLine(String.Format("识别物体类型:场景识别, 识别名称:%s, 分数:%s", discern, rate));
                                        break;
                                    case 2:
                                        Console.WriteLine(String.Format("识别物体类型:地标识别, 识别名称:%s, 分数:%s", discern, rate));
                                        break;
                                    case 3:
                                        Console.WriteLine(String.Format("识别物体类型:百度识图, 识别名称:%s, 分数:%s", discern, rate));
                                        break;
                                    default:
                                        break;
                                }
                            }
                        }
                        // 用户画像结果
                        if(null!=tmp.GetValue("userRisk")){
                            JObject userRisk = tmp.GetValue("userRisk").ToObject<JObject>();
                            JArray userRiskDetails = (JArray)userRisk.SelectToken("details");
                            foreach (var userRiskDetail in userRiskDetails)
                            {
                                JObject lObject = (JObject)userRiskDetail;
                                int accountLevel = lObject.GetValue("accountLevel").ToObject<int>();
                                String account = lObject.GetValue("account").ToObject<String>();
                                switch (accountLevel) {
                                    case 0:
                                        Console.WriteLine(String.Format("账号:{0}, 用户账号风险级别:正常", account));
                                        break;
                                    case 5:
                                        Console.WriteLine(String.Format("账号:{0}, 用户账号风险级别:嫌疑", account));
                                        break;
                                    case 10:
                                        Console.WriteLine(String.Format("账号:{0}, 用户账号风险级别:致命", account));
                                        break;
                                    default:
                                        break;
                                }
                            }
                        }
                    }
                }
                else{
                    Console.WriteLine(String.Format("ERROR: code={0}, msg={1}", code, msg));
                }
            }else{
                Console.WriteLine("Request failed!");        
            }
        }
    }
}